...
public class MonitorUtil {
    ...
    static MonitorInfo getMonitorInfoBean() throws Exception {
        ...
        for (long id : ids) {
            threadTimes.put("threadCpuTime." + id, new BigDecimal(threadMXBean.getThreadCpuTime(id) / min).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue());
            threadTimes.put("threadUserTime." + id, new BigDecimal(threadMXBean.getThreadUserTime(id) / min).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue());
        }
