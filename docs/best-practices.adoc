...
TIP:  接口抛出异常经过异常处理接口解析后，feign解码返回值时会在消费端报 `feign.codec.DecodeException` 异常，需在FeignClient接口上加 `@RequestMapping(produces = MediaType.APPLICATION_JSON_UTF8_VALUE)` ，指定响应类型和编码集。
...
WARNING: `hystrix` 的默认超时时间为5s,在高并发下容易出现超时,建议将默认超时时间适当调长,
