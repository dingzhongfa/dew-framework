...
TIP: 在`@FeignClient`修饰类中的接口方法里添加新的形参，并加上@RequestHeader注解指定key值
...
=== `SpringBoot（SpringCloud）jar` 包扫描配置文件顺序

* bootstrap.yml(jar包外) > bootstrap.yml(jar包内) > application.yml(jar包外) > application.yml(jar包内)

* 当启用Spring Cloud Config 配置后，Git仓库中的application.yml > [application-name].yml > [application-name]-[profile].yml

TIP: 配置的覆盖不是以文件为单位，而是以配置中的参数为单位。

TIP: 同一参数取最先扫描到的value！

TIP: 建议配置同一配置不要重复配置，很不规范，尤其是生产环境中。