...
在需要使用的幂等处理的请求头或参数中加上配置中指定的两个key(``操作类型标识``及``幂等操作ID标识``，两者构成唯一)，同时还可以指定一些额外的配置（见上文配置）。

当``是否需要确认``为true时，需要进行显式确认操作: ``DewIdempotent.confirm() 或 DewIdempotent.confirm(String optType, String optId)`` 前者要求与请求入口在同一线程中。
